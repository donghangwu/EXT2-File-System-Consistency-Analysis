...
class Group:
    def __init__(self, data):
        ...
        # starting block# for free block entries
        self.block_bitmap = int(data[6])
        # starting block# for free node entries
        self.inode_bitmap = int(data[7])
        # starting block# for first block of inodes
        self.inode_table = int(data[8])
...
class DIRENT:
    def __init__(self, data):
        ...
        self.logical_offset = int(data[2])
        self.node_num = int(data[3])
        self.rec_len = int(data[4])
        self.name_len = int(data[5])
        self.name = data[6]
...
# print(group.inode_table)
    legal_block = int(group.inode_table) + \
        int((superBlock.inode_size*group.inodes_count)/superBlock.bsize)
    ...
    for node in inodes:
        ...
        for block in node.dir_block:
            ...
            if(block not in dup_blocks):
                ...
                print("DUPLICATE BLOCK", block, "IN INODE",
                      node.node_num, "AT OFFSET", 0)
            ...
            if(int(node.indir_block[0]) != 0):
                if(int(node.indir_block[0]) < 0 or int(node.indir_block[0]) > superBlock.blocks_count):
                    ...
                    print("INVALID INDIRECT BLOCK",
                          node.indir_block[0], "IN INODE", node.node_num, "AT OFFSET", 12)
                # reserved block
                ...
                if(node.indir_block[0] in bfree):
                    ...
                    print("ALLOCATED BLOCK",
                          node.indir_block[0], "ON FREELIST")

                if(node.indir_block[0] not in dup_blocks):
                    dup_blocks[node.indir_block[0]] = [
                        1, node.node_num, "INDIRECT BLOCK", 12]
                else:
                    print("DUPLICATE INDIRECT BLOCK",
                          node.indir_block[0], "IN INODE", node.node_num, "AT OFFSET", 12)
            ...
            if(int(node.indir_block[1]) != 0):
                if(int(node.indir_block[1]) < 0 or int(node.indir_block[1]) > superBlock.blocks_count):
                    ...
                    print("INVALID DOUBLE INDIRECT BLOCK",
                          node.indir_block[1], "IN INODE", node.node_num, "AT OFFSET", 268)
                # reserved block
                ...
                if(node.indir_block[1] in bfree):
                    fail = True
                    print("ALLOCATED BLOCK",
                          node.indir_block[1], "ON FREELIST")
                # dup
                if(node.indir_block[1] not in dup_blocks):
                    dup_blocks[node.indir_block[1]] = [
                        1, node.node_num, "DOUBLE INDIRECT BLOCK", 268]
                else:
                    print("DUPLICATE DOUBLE INDIRECT BLOCK",
                          node.indir_block[1], "IN INODE", node.node_num, "AT OFFSET", 268)
            ...
            if(int(node.indir_block[2]) != 0):
                if(int(node.indir_block[2]) < 0 or int(node.indir_block[2]) > superBlock.blocks_count):
                    ...
                    print("INVALID TRIPLE INDIRECT BLOCK",
                          node.indir_block[2], "IN INODE", node.node_num, "AT OFFSET", 65804)
                # reserved block
                ...
                if(node.indir_block[2] in bfree):
                    fail = True
                    print("ALLOCATED BLOCK",
                          node.indir_block[2], "ON FREELIST")

                if(node.indir_block[2] not in dup_blocks):
                    dup_blocks[node.indir_block[2]] = [
                        1, node.node_num, "TRIPLE INDIRECT BLOCK", 65804]
                else:
                    print("DUPLICATE TRIPLE INDIRECT BLOCK",
                          node.indir_block[2], "IN INODE", node.node_num, "AT OFFSET", 65804)
    ...
    for block in dup_blocks:
        if(dup_blocks[block][0] > 1):
            fail = True
            print("DUPLICATE", dup_blocks[block][2], block, "IN INODE",
                  dup_blocks[block][1], "AT OFFSET", dup_blocks[block][3])
    ...
    # keep track of the referenced times for each inode, compare with i_link_count
    node_links = Counter()  # counter: default 0 to avoid unreferenced errorstency Audits
    ...
    for entry in dirents:
        ...
        if entry.node_num < 1 or entry.node_num > superBlock.inodes_count:
            print(
                f"DIRECTORY INODE {entry.parent} NAME {entry.name} INVALID INODE {entry.node_num}")
            fail = True
            continue
        if entry.node_num not in allocated:
            print(
                f"DIRECTORY INODE {entry.parent} NAME {entry.name} UNALLOCATED INODE {entry.node_num}")
            fail = True
            continue
        elif entry.name == "'.'" and entry.parent != entry.node_num:
            print(
                f"DIRECTORY INODE {entry.parent} NAME '.' LINK TO {entry.node_num} SHOULD BE {entry.parent}")
            fail = True
        elif entry.name == "'..'" or entry.name == "'.'":
            continue
        else:
    ...
    for entry in dirents:
        ...
        if entry.name == "'..'" and entry.node_num != parent:
            fail = True
            print(
                f"DIRECTORY INODE {entry.parent} NAME '..' LINK TO INODE {entry.node_num} SHOULD BE {parent}")

    for node in inodes:
        links = node_links[str(node.node_num)]
        if links != node.i_links_count:
            fail = True
            print(
                f"INODE {node.node_num} HAS {links} LINKS BUT LINKCOUNT IS {node.i_links_count}")
